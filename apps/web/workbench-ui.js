...
(() => {
  ...
  function renderBriefing() {
    const briefing = wb.briefing || {};
    const counts = briefing.counts || {};
    const focus = briefing.focus || [];
    const schedule = briefing.schedule || [];
    const drafts = briefing.ready_drafts || [];
    const learning = briefing.learning || {};
    const mailbox = dashboard.mailboxes[0];
    const behavior = runtimeSettings?.behavior || {};
    const kindLabel = value => ({decision:'Entscheidung',security:'Sicherheit',approval:'Freigabe',recovery:'Prüfen',follow_up:'Antwort',calendar_approval:'Termin'})[value] || 'Aufgabe';
    const kindClass = value => ['security','recovery'].includes(value) ? 'red' : value === 'calendar_approval' ? 'green' : 'warm';
  ...
  async function loadDailyBriefing(silent=true, force=false) {
    if (wb.briefingLoading || (!force && wb.briefing && Date.now()-wb.briefingLoadedAt<60000)) return;
    wb.briefingLoading = true;
    try { wb.briefing = await get('/v1/briefing?limit=20'); wb.briefingLoadedAt=Date.now(); }
    catch (error) { if (!silent) showNotice(error.message,'error'); }
    finally { wb.briefingLoading = false; }
  }
  ...
  bindDashboard = function workbenchBindDashboard() {
    ...
    document.querySelectorAll('[data-view]').forEach(el => {
      el.onclick = async event => {
        ...
        if (view === 'overview') await loadDailyBriefing(true);
  ...
  function bindWorkbench() {
    ...
    document.querySelectorAll('[data-attention-instruct]').forEach(button=>button.addEventListener('click',async()=>{
      const id=button.dataset.attentionInstruct, mailboxId=button.dataset.mailbox;
      const instruction=document.querySelector(`[data-attention-note="${CSS.escape(id)}"]`)?.value?.trim()||'';
      if(!instruction){showNotice('Schreibe dem Agenten zuerst, was er tun soll.','error');return;}
      button.disabled=true;
      showNotice('Der Agent verarbeitet deine Anweisung …');
      try{
        const response=await post('/v1/attention/instruct',{mailbox_id:mailboxId,message_id:id,instruction,actor:'local-user'});
        await Promise.all([loadAttention(true),loadDashboard(true),loadDailyBriefing(true,true)]);
        if(response.accepted && response.next_view){
          activeView=response.next_view;
          if(response.approval_id){const i=(dashboard.approvals||[]).findIndex(x=>String(x.approval_id)===String(response.approval_id));if(i>=0)wb.selectedApproval=i;}
          if(response.draft_id){const i=(dashboard.drafts||[]).findIndex(x=>String(x.draft_id)===String(response.draft_id));if(i>=0)wb.selectedDraft=i;}
          history.replaceState({},'',activeView==='overview'?'/' : `/?view=${encodeURIComponent(activeView)}`);
        }
        showNotice(response.message||'Der Agent hat deine Anweisung verarbeitet.',response.accepted?'success':'error');
        render();
  ...
  setTimeout(async()=>{ if (installed) { await Promise.all([loadDailyBriefing(true),loadAttention(true),loadConversationIntelligence(true)]); render(); } }, 450);
})();
