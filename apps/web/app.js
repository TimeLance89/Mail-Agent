...
function renderAgentSettings(){
  ...
  return `<div class="settings-grid agent-settings-grid">
    ...
    <section class="panel"><div class="panel-head"><div><span>SICHERHEIT</span><h3>Unverhandelbare Grenzen</h3></div></div><div class="security-block">${icon('lock',22)}<div><b>Credential Vault aktiv</b><p>Mailbox-Secrets, OAuth-Tokens und Agent-Schlüssel bleiben lokal geschützt.</p></div></div><div class="security-block">${icon('shield',22)}<div><b>Policy Engine vor Ausführung</b><p>Das LLM schlägt Aktionen nur vor. Regeln, Freigaben und Agent-ID-Pflicht werden deterministisch im Gateway erzwungen.</p></div></div><div class="security-block">${icon('sync',22)}<div><b>Einrichtung neu beginnen</b><p>Startet das Onboarding erneut und pausiert den Agenten. Identität, Postfachverbindungen und betriebliche Historie bleiben erhalten; Profil, Autonomie-Einstellungen und optionale Lerndaten werden entfernt.</p><div class="inline-actions left"><button class="btn secondary" id="settings-reset-onboarding">Onboarding zurücksetzen</button></div></div></div></section>
  </div>`;
}
...
async function resetOnboarding(){
  const confirmed=window.confirm('Onboarding wirklich zurücksetzen? Der Agent wird pausiert. Profil, Autonomie-Einstellungen und optionale Lerndaten werden gelöscht; Identität, Postfachverbindungen und betriebliche Historie bleiben erhalten.');
  if(!confirmed)return;
  try{
    await post('/v1/onboarding/reset',{confirmation:'RESET ONBOARDING',actor:'local-user'});
    installed=false;step=0;runtimeSettings=null;brainStatus=null;
    showNotice('Einrichtung wurde zurückgesetzt. Das Onboarding startet neu.');
...
function renderUpdatePanel(){
  ...
  if(!grid)return;
  const current=updateStatus?.current_version||'0.18.2';
...
function bindDashboard(){
  ...
  document.getElementById('settings-reset-onboarding')?.addEventListener('click',resetOnboarding);
