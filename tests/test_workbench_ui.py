...
def test_workbench_is_first_class_and_adaptive_assets_are_loaded():
    ...
    assert "/assets/workbench.css?v=0.19.1" in index
    assert "/assets/workbench-ui.js?v=0.19.1" in index
    assert "/assets/adaptive-intelligence.css?v=0.19.1" in index
    assert "/assets/adaptive-intelligence-ui.js?v=0.19.1" in index
    assert "/assets/calendar-ui.js?v=0.19.1" in index
    assert "/assets/dashboard-live.js?v=0.19.1" in index
...
def test_calendar_ui_is_approval_gated_and_has_no_recursive_observer():
    source = (ROOT / "apps/web/calendar-ui.js").read_text(encoding="utf-8")
    for marker in (
