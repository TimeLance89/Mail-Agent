...
def test_inno_installer_passes_version_and_restart_mode_to_verifier():
    ...
    assert '#define MyAppVersion "0.19.1"' in source
...
def test_restart_helper_accepts_only_expected_healthy_version(tmp_path: Path):
    ...
    command_line = subprocess.list2cmdline(
        [str(RESTART_HELPER), str(sys.executable), "0.19.1", "no-browser"]
    )
    try:
    ...
    assert log.exists()
    assert "Gateway is reachable with expected version 0.19.1" in log.read_text(
