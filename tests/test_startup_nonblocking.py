...
def test_startup_guard_runs_immediately_after_main_app():
    ...
    assert index.index("/assets/app.js?v=0.15.0") < index.index(
        "/assets/startup-rescue.js?v=0.15.0"
    )
    assert index.index("/assets/startup-rescue.js?v=0.15.0") < index.index(
        "/assets/workbench-ui.js?v=0.15.0"
    )
    assert index.index("/assets/workbench-ui.js?v=0.15.0") < index.index(
        "/assets/mail-provider-setup.js?v=0.15.0"
    )
    assert index.index("/assets/mail-provider-setup.js?v=0.15.0") < index.index(
        "/assets/llm-model-settings-v2.js?v=0.15.0"
    )
...
def test_every_web_asset_is_cache_busted_for_the_hotfix():
    ...
    for asset in (
        ...
        assert f"/assets/{asset}?v=0.15.0" in index
