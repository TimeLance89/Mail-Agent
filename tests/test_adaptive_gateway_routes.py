...
def test_adaptive_calendar_and_draft_routes_remain_reachable_with_web_bundle_mounted():
    ...
    assert status.json()["version"] == "0.18.1"
...
def test_web_root_and_assets_mount_remain_available_after_route_reordering():
    ...
    assert "/assets/calendar-ui.js?v=0.18.1" in root.text
    assert "/assets/v171-ux.js?v=0.18.1" in root.text
    assert "/assets/v172-ux.js?v=0.18.1" in root.text
