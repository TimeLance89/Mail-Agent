...
def test_v172_gateway_loads_only_after_frozen_environment_is_authoritative():
    entry = (ROOT / "apps/launcher/mail_agent_launcher/v172_entry.py").read_text(encoding="utf-8")
    configure = "launcher.configure_environment(data_dir)"
    gateway_import = "from mail_agent_gateway import main_v172 as _gateway_v172"
...
def test_release_version_is_synchronized_for_0172():
    ...
    assert 'version = "0.17.2"' in pyproject
    assert 'mail-agent = "mail_agent_launcher.v172_entry:main"' in pyproject
    assert 'APP_VERSION = "0.17.2"' in gateway_v172
    assert 'APP_VERSION = "0.17.2"' in launcher_v172
    assert "from mail_agent_launcher.v172_entry import main" in launcher_entry
    assert 'app_version: str = "0.17.2"' in identity
    assert '#define MyAppVersion "0.17.2"' in installer
    assert "const APP_VERSION = '0.17.2'" in desktop
    assert '"version": "0.17.2"' in codex
    assert "/assets/startup-rescue.js?v=0.17.2" in index
    ...
    assert "/assets/desktop-links.js?v=0.17.2" in index
